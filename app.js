...
import { renderCurriculumOutline, renderChapterNavigation, chapterUrl } from './curriculum-ui.js';
...
function subjectOverview(kind) {
 ...
 if(isTheory) return head('VOLUME 01 · THEORY',curriculum.title,'PART 01부터 PART 03까지, 18개 CHAPTER를 순서대로 학습하세요.')+renderCurriculumOutline(state.completed);
...
function route(){
 ...
 activeLesson=page==='theory' && filter ? lessons.find(l=>l.id===resolveLessonId(lessonId) && l.subject===filter) || lessons.find(l=>l.subject===filter) : null;
 if(activeLesson){state.lastLesson=activeLesson.id;save();if(lessonId && resolveLessonId(lessonId)!==lessonId) history.replaceState(null,'',chapterUrl(activeLesson));}
 document.querySelector('#modal').close();
