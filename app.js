...
import { findChapter, practiceQuestions, renderChapterFilter, renderPracticeGroups, renderQuestionExplanation } from './practice-ui.js';
...
function questionRow(q) {
 ...
 return `<article class="question-row card"><span class="question-index">${String(q.id).padStart(2,'0')}</span><button class="question-title" data-action="single-question" data-id="${q.id}"><div><span class="subject-label">제${q.subject}과목</span><span class="difficulty">${q.core?'단원별 핵심문제':q.difficulty}</span>${answered ? `<span class="answer-status ${correct?'correct':'incorrect'}">${correct?'정답':'복습 필요'}</span>` : ''}</div><h3>${q.text}</h3><p>${chapterLabel(lessons.find(l=>l.id===q.lesson).chapter)} · ${lessons.find(l=>l.id===q.lesson).title}</p></button><button class="icon-button ${state.bookmarks.includes('q'+q.id)?'bookmarked':''}" data-action="bookmark-question" data-id="${q.id}" aria-label="${q.id}번 문제 북마크" aria-pressed="${state.bookmarks.includes('q'+q.id)}">${icon('bookmark')}</button><button class="round-arrow" data-action="single-question" data-id="${q.id}" aria-label="${q.id}번 문제 풀기">${icon('arrow')}</button></article>`;
}
