...
const initial = () => ({ completed: [], answers: {}, bookmarks: [], activity: {}, goal: 5, examDate: '', lastLesson: null, exams: [] });
let state = initial();
try {
 const saved = JSON.parse(localStorage.getItem('jodalon-v1'));
 if (saved && Array.isArray(saved.completed) && Array.isArray(saved.bookmarks) && saved.answers && saved.activity) state = { ...state, ...saved };
...
const navItems = [['dashboard','home','학습 대시보드'],['theory','book','기본이론'],['practice','pen','예상문제'],['mock','clock','모의고사'],['wrong','file','오답노트'],['saved','bookmark','북마크']];
...
function dashboard() {
 ...
 <div class="dashboard-columns"><div class="primary-column">
  ...
  <section class="section"><div class="section-title"><h2>과목별 기본이론 <span class="count-pill">3과목</span></h2><a href="#theory">전체 보기 ${icon('chevron')}</a></div><div class="subject-grid">${subjects.map(subjectCard).join('')}</div></section>
 ...
 </aside></div><div class="source-note">${icon('info')} 기본 개념을 익히는 입문 학습 콘텐츠와 자체 제작 예상문제를 제공합니다. 최신 출제기준과 법령은 공식 자료를 함께 확인해 주세요.</div>`;
}
function weekDays() {
...
function subjectCard(s) { const done = lessons.filter(l=>l.subject===s.id && state.completed.includes(l.id)).length; return `<article class="subject-card"><div class="subject-card-top"><span class="subject-icon ${s.color}">${icon(s.icon)}</span><span>제${s.id}과목</span></div><h3>${s.title}</h3><p>${s.desc}</p><div class="tags">${s.tags.map(t=>`<span>${t}</span>`).join('')}</div><div class="subject-progress"><span>학습 진도 <b>${done}/4</b></span><strong>${done*25}%</strong></div><div class="progress-track"><span style="width:${done*25}%"></span></div><button data-action="subject" data-id="${s.id}">학습 시작하기 ${icon('arrow')}</button></article>`; }
function tabs() { return `<div class="tabs" role="group" aria-label="과목 선택">${[{id:0,title:'전체 과목'},...subjects].map(s=>`<button class="${filter===s.id?'selected':''}" data-action="filter" data-id="${s.id}" aria-pressed="${filter===s.id}">${s.id ? `제${s.id}과목 · ${s.short}` : s.title}</button>`).join('')}</div>`; }
...
function subjectOverview(kind) {
 ...
 return '<a class="card library-card" href="#'+kind+'/'+s.id+'"><span class="subject-icon '+s.color+'">'+icon(s.icon)+'</span><div><span class="eyebrow">SUBJECT 0'+s.id+'</span><h2>'+s.title+'</h2><p>'+s.desc+'</p><span class="library-meta">'+(isTheory?'기본이론 '+count+'단원':'예상문제 '+count+'문항 · 정답 및 해설')+'</span></div><span class="library-open">'+(isTheory?'이론 학습하기':'문제 보러 가기')+' '+icon('arrow')+'</span></a>';
 }).join('')+'</div>';
}
function theory() {
 if(!filter) return subjectOverview('theory');
 const s=subjects.find(s=>s.id===filter), l=activeLesson || lessons.find(l=>l.subject===filter);
 const list=lessons.filter(item=>item.subject===filter), index=list.findIndex(item=>item.id===l.id);
 return head('SUBJECT 0'+s.id+' · THEORY',s.title,'제'+s.id+'과목 · 기본이론 '+list.length+'단원', '<a class="soft-button" href="#practice/'+s.id+'">'+icon('pen')+' 이 과목 예상문제</a>')+
 tabs()+'<div class="reading-layout"><aside class="card chapter-nav"><h2>학습 목차</h2>'+list.map((item,i)=>'<a class="'+(item.id===l.id?'current':'')+'" href="#theory/'+s.id+'/'+item.id+'"><span>'+String(i+1).padStart(2,'0')+'</span><div>'+item.title+'<small>'+item.minutes+'분'+(state.completed.includes(item.id)?' · 학습 완료':'')+'</small></div>'+icon(state.completed.includes(item.id)?'check':'chevron')+'</a>').join('')+'<a class="chapter-practice" href="#practice/'+s.id+'">'+icon('pen')+' 예상문제 페이지 '+icon('arrow')+'</a></aside><div class="card reading-card"><article class="lesson-content"><div class="eyebrow">CHAPTER '+l.id.replace('-','.')+'</div><h2>'+l.title+'</h2><p class="lesson-summary">'+l.summary+'</p><div class="lesson-tools"><span>'+icon('clock')+' 약 '+l.minutes+'분</span><button class="soft-button '+(state.bookmarks.includes('l'+l.id)?'bookmarked':'')+'" data-action="bookmark-lesson" data-id="'+l.id+'">'+icon('bookmark')+' '+(state.bookmarks.includes('l'+l.id)?'저장됨':'북마크')+'</button></div>'+l.sections.map(([title,content],i)=>'<section><h3><span>'+String(i+1).padStart(2,'0')+'</span>'+title+'</h3><p>'+content+'</p></section>').join('')+renderLessonGuide(l)+'<div class="takeaway"><strong>'+icon('info')+' 핵심 정리</strong><p>'+l.takeaway+'</p></div><div class="lesson-source">관련 공식 자료 '+external(sources[l.source],l.source==='pps'?'공공조달역량개발원 표준교재':'국가법령정보센터')+'</div><p class="muted small-text">자체 작성한 입문용 학습 자료입니다. 상세 요건은 현행 법령과 공식 교재를 확인하세요.</p></article><div class="reading-actions"><span>'+(index+1)+' / '+list.length+'단원</span><button class="button" data-action="complete-lesson" data-id="'+l.id+'">'+icon('check')+' '+(state.completed.includes(l.id)?'학습 완료됨':'학습 완료하기')+'</button></div><div class="chapter-pagination">'+(index>0?'<a href="#theory/'+s.id+'/'+list[index-1].id+'">← 이전 단원</a>':'<span></span>')+(index<list.length-1?'<a href="#theory/'+s.id+'/'+list[index+1].id+'">다음 단원 →</a>':'<a href="#practice/'+s.id+'">예상문제로 확인하기 →</a>')+'</div></div></div>';
}
...
function lessonList() {
 ...
 return subjects.filter(s=>filtered.some(l=>l.subject===s.id)).map(s=>`<section class="section lesson-group"><div class="section-title"><h2><span class="subject-icon ${s.color}">${icon(s.icon)}</span> 제${s.id}과목. ${s.title}</h2><span>${filtered.filter(l=>l.subject===s.id).length}개 단원</span></div><div class="card">${filtered.filter(l=>l.subject===s.id).map(l=>`<div class="lesson-row"><span class="lesson-number ${state.completed.includes(l.id)?'complete':''}">${state.completed.includes(l.id)?icon('check'):l.id.replace('-','.')}</span><button class="lesson-title" data-action="lesson" data-id="${l.id}"><h3>${l.title}</h3><p>${l.summary}</p></button><span class="duration">${icon('clock')} ${l.minutes}분</span><button class="icon-button ${state.bookmarks.includes('l'+l.id)?'bookmarked':''}" data-action="bookmark-lesson" data-id="${l.id}" aria-label="${esc(l.title)} 북마크" aria-pressed="${state.bookmarks.includes('l'+l.id)}">${icon('bookmark')}</button><button class="soft-button" data-action="lesson" data-id="${l.id}">${state.completed.includes(l.id)?'다시 읽기':'학습하기'} ${icon('chevron')}</button></div>`).join('')}</div></section>`).join('');
}
function practice() {
 if (!filter) return subjectOverview('practice');
 const filtered = questions.filter(q=>!filter || q.subject===filter);
 return `${head('TURN KNOWLEDGE INTO SKILL','제'+filter+'과목 · 예상문제',subjects[filter-1].title+' · 문제를 풀고 해설을 확인하세요.',`<button class="button" data-action="start-filtered">${icon('pen')} ${filter?'선택 과목':'전체'} 풀기</button>`)}<div class="notice-line">${icon('info')} 공식 기출이 아닌 자체 제작 입문·개념 확인용 문항 ${filtered.length}개입니다.</div>${tabs()}<div class="question-list">${filtered.map(questionRow).join('')}</div>`;
}
function questionRow(q) {
 const answered = state.answers[q.id] !== undefined;
 const correct = state.answers[q.id]===q.answer;
 return `<article class="question-row card"><span class="question-index">${String(q.id).padStart(2,'0')}</span><button class="question-title" data-action="single-question" data-id="${q.id}"><div><span class="subject-label">제${q.subject}과목</span><span class="difficulty">${q.difficulty}</span>${answered ? `<span class="answer-status ${correct?'correct':'incorrect'}">${correct?'정답':'복습 필요'}</span>` : ''}</div><h3>${q.text}</h3><p>${lessons.find(l=>l.id===q.lesson).title}</p></button><button class="icon-button ${state.bookmarks.includes('q'+q.id)?'bookmarked':''}" data-action="bookmark-question" data-id="${q.id}" aria-label="${q.id}번 문제 북마크" aria-pressed="${state.bookmarks.includes('q'+q.id)}">${icon('bookmark')}</button><button class="round-arrow" data-action="single-question" data-id="${q.id}" aria-label="${q.id}번 문제 풀기">${icon('arrow')}</button></article>`;
}
...
function renderQuiz() {
 ...
 document.querySelector('#main').innerHTML = `${head(quiz.mode==='exam'?'MINI MOCK EXAM':'PRACTICE MAKES PROGRESS',quiz.mode==='exam'?'미니 모의고사':'예상문제 풀이','한 문제씩 집중하며, 배운 내용을 확인해 보세요.',`<button class="soft-button" data-action="exit-quiz">그만 풀기 ${icon('close')}</button>`)}<div class="quiz-layout"><section class="card quiz-card"><div class="quiz-top"><span class="subject-label">제${q.subject}과목 · ${subjects[q.subject-1].short}</span><span>${quiz.index+1} <span class="muted">/ ${quiz.list.length}문제</span></span></div><div class="progress-track"><span style="width:${(quiz.index+1)/quiz.list.length*100}%"></span></div><div class="quiz-question"><span class="difficulty">${q.difficulty}</span><button class="icon-button ${state.bookmarks.includes('q'+q.id)?'bookmarked':''}" data-action="bookmark-question" data-id="${q.id}" aria-label="문제 북마크" aria-pressed="${state.bookmarks.includes('q'+q.id)}">${icon('bookmark')}</button><h2><span>Q${String(quiz.index+1).padStart(2,'0')}.</span> ${q.text}</h2></div><div class="options" role="group" aria-label="답안 선택">${q.options.map((option,i)=>`<button class="option ${selected===i?'chosen':''} ${revealed && q.answer===i?'right':''} ${revealed && selected===i && q.answer!==i?'wrong':''}" data-action="select-answer" data-id="${i}" aria-pressed="${selected===i}" ${revealed?'disabled':''}><span>${i+1}</span><span>${option}</span>${revealed && q.answer===i ? icon('check'):''}</button>`).join('')}</div>${revealed ? `<div class="explanation ${selected===q.answer?'success':'error'}" role="status"><strong>${icon(selected===q.answer?'check':'info')} ${selected===q.answer?'정답이에요!':'조금 아쉬워요. 정답은 '+(q.answer+1)+'번이에요.'}</strong><p>${q.explanation}</p><button class="text-button" data-action="lesson" data-id="${q.lesson}">관련 이론 읽기 ${icon('arrow')}</button></div>`:''}<div class="quiz-bottom"><button class="soft-button" data-action="prev-question" ${quiz.index===0?'disabled':''}>이전 문제</button>${quiz.mode==='exam' ? `<button class="button" data-action="${quiz.index===quiz.list.length-1?'submit-exam':'next-question'}">${quiz.index===quiz.list.length-1?'답안 제출하기':'다음 문제'} ${icon('arrow')}</button>` : revealed ? `<button class="button" data-action="next-question">${quiz.index===quiz.list.length-1?'학습 결과 보기':'다음 문제'} ${icon('arrow')}</button>` : `<button class="button" data-action="check-answer" ${selected===null?'disabled':''}>정답 확인하기 ${icon('check')}</button>`}</div></section><aside class="card quiz-side"><h3>${quiz.mode==='exam'?icon('clock')+' 남은 시간':'문제 한눈에 보기'}</h3>${quiz.mode==='exam'?'<div class="timer" id="timer"></div>':''}<div class="question-dots">${quiz.list.map((item,i)=>`<button class="${i===quiz.index?'current':''} ${quiz.answers[item.id]!==undefined?'answered':''}" data-action="jump-question" data-id="${i}" aria-label="${i+1}번 문제로 이동">${i+1}</button>`).join('')}</div><p>${quiz.mode==='exam'?'답안은 제출 후 채점됩니다. 미응답 문항은 오답으로 처리됩니다.':'답안을 확인하면 학습 기록에 저장돼요. 오답은 다시 풀며 복습해 보세요.'}</p><span class="small-text muted">자체 제작 개념 확인용 문항</span></aside></div>`;
...
document.addEventListener('click',event=>{
 ...
 if(action==='lesson-quiz') go('practice/'+Number(id[0]));
 if(action==='single-question') startQuiz([questions.find(q=>q.id===Number(id))]);
 if(action==='start-filtered') startQuiz(questions.filter(q=>!filter||q.subject===filter));
...
document.addEventListener('input',event=>{if(event.target.id==='lesson-search'){query=event.target.value;document.querySelector('#lesson-list').innerHTML=lessonList();}});
...
function route(){
 ...
 activeLesson=page==='theory' && filter ? lessons.find(l=>l.id===lessonId && l.subject===filter) || lessons.find(l=>l.subject===filter) : null;
