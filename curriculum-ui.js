...
export function renderPartDirectory(completed = []) {
  return `<div class="subject-library part-directory">${subjects.map(s => {
    const list = lessons.filter(l => l.subject === s.id);
    const done = list.filter(l => completed.includes(l.id)).length;
    return `<a class="card library-card" href="#theory/${s.id}"><div><span class="subject-label">${partLabel(s.id)}</span><h2>${esc(s.title)}</h2><p>${esc(s.desc)}</p><span class="library-meta">${list.length}개 CHAPTER · ${done}개 학습 완료</span></div><span class="library-open">PART 목차 열기 →</span></a>`;
  }).join('')}</div>`;
}

export function renderCurriculumOutline(completed = [], subjectId = 0) {
  return `<div class="curriculum-outline" aria-label="${subjectId ? partLabel(subjectId) : esc(curriculum.title)} 목차">${subjects.filter(s => !subjectId || s.id === subjectId).map(s => {
    const list = lessons.filter(l => l.subject === s.id);
...
export function renderChapterNavigation(activeLesson, completed = []) {
  return `<aside class="card chapter-nav curriculum-nav"><a class="curriculum-volume" href="#theory/${activeLesson.subject}">← ${partLabel(activeLesson.subject)} 목차</a>${subjects.filter(s => activeLesson.subject === s.id).map(s => `<details open><summary><span>${partLabel(s.id)}</span><strong>${esc(s.title)}</strong></summary>${lessons.filter(l=>l.subject===s.id).map(l => `<a class="${l.id===activeLesson.id?'current':''}" ${l.id===activeLesson.id?'aria-current="page"':''} href="${chapterUrl(l)}"><span class="chapter-code">${chapterLabel(l.chapter)}</span><strong>${esc(l.title)}</strong><small>약 ${l.minutes}분${completed.includes(l.id)?' · 학습 완료':''}</small></a>`).join('')}</details>`).join('')}<a class="chapter-practice" href="#practice/${activeLesson.subject}/${activeLesson.id}">이 CHAPTER의 예상문제 →</a></aside>`;
}
